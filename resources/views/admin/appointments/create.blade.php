                ...
                <div class="row">
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}"
                            name="client_id" id="client_id" required onchange="getContracts()">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('contract') ? 'is-invalid' : '' }}"
                            name="contract_id" id="contract_id" required>
                            {{-- ajax --}}
                        </select>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control {{ $errors->has('malfunction_type') ? 'is-invalid' : '' }}" name="malfunction_type_id"
                            ...
                            @foreach ($malfunctionTypes as $id => $malfunctionType)
                                <option value="{{ $id }}"
                                    {{ old('malfunction_type_id') == $id ? 'selected' : '' }}>{{ $malfunctionType }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control select2 {{ $errors->has('technicians') ? 'is-invalid' : '' }}"
                            name="technicians[]" id="technicians" multiple>
                            @foreach ($technicians as $id => $technician)
                                <option value="{{ $id }}"
                                    {{ in_array($id, old('technicians', [])) ? 'selected' : '' }}>{{ $technician }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        <label
                            for="problem_description">{{ trans('cruds.appointment.fields.problem_description') }}</label>
                        <textarea class="form-control {{ $errors->has('problem_description') ? 'is-invalid' : '' }}" name="problem_description"
                            id="problem_description">{{ old('problem_description') }}</textarea>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <div class="needsclick dropzone {{ $errors->has('problem_photos') ? 'is-invalid' : '' }}"
                            id="problem_photos-dropzone">
                        </div>
...
@section('scripts')
    <script>
        function getContracts(){
            $.ajax({
                type: 'POST',
                url: '{{ route('admin.appointments.get_contracts') }}',
                data: {
                    _token: '{{ csrf_token() }}',
