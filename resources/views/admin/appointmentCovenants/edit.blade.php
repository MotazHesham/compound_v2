...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.appointmentCovenant.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointment-covenants.update", [$appointmentCovenant->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('covenant') ? 'is-invalid' : '' }}" name="covenant_id" id="covenant_id">
                    @foreach($covenants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('covenant_id') ? old('covenant_id') : $appointmentCovenant->covenant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $appointmentCovenant->quantity) }}" step="1">
