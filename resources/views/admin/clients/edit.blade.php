...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clients.update", [$client->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="user_id" value="{{ $client->user_id }}" id="">
            <div class="row">
                ...
                <div class="form-group col-md-3">
                    ...
                    <input class="form-control {{ $errors->has('phone_2') ? 'is-invalid' : '' }}" type="text" name="phone_2" id="phone_2" value="{{ old('phone_2', $client->phone_2) }}">
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control select2 {{ $errors->has('property_type') ? 'is-invalid' : '' }}" name="property_type_id" id="property_type_id" required>
                        @foreach($property_types as $id => $entry)
                            <option value="{{ $id }}" {{ old('property_type_id',$client->property_type_id) == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control {{ $errors->has('client_status') ? 'is-invalid' : '' }}" name="client_status" id="client_status" required>
                        ...
                        @foreach(App\Models\Client::CLIENT_STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('client_status', $client->client_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control select2 {{ $errors->has('nationality') ? 'is-invalid' : '' }}"
                        ...
                        @foreach (App\Models\User::NATIONALITY_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('nationality', $user->nationality) === (string) $key ? 'selected' : '' }}>{{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-3">
                    ...
                    <input class="form-control {{ $errors->has('identity_num') ? 'is-invalid' : '' }}" type="text"
                        name="identity_num" id="identity_num" value="{{ old('identity_num', $user->identity_num) }}" required>
